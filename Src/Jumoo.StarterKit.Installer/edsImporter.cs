...
namespace Jumoo.StarterKit.Installer
{
    ...
    public static class ImporterConstants
    {
        ...
        public const string ContentBody = "content";
        public const string ContentSummary = "summary";
    ...
    public class esdImporter
    {
        ...
        {
            string file = IOHelper.MapPath("~/app_data/lgsk/" + fileName);
        ...
        public int CreateContent(string template, XElement node, int parent)
        {
            ...
            if (content != null)
            {
                if (content.HasProperty(ImporterConstants.ContentTitle))
                    SetContentValue(content, ImporterConstants.ContentTitle, title);


                if (content.HasProperty(ImporterConstants.ContentEsdId))
                    SetContentValue(content, ImporterConstants.ContentEsdId, esdId);

                if (content.HasProperty(ImporterConstants.ContentSummary))
        ...
        private IContent MakeHomeNode()
        {
            ...
            if (home != null)
            {
                home.SetValue("title", "Welcome");
                // home.SetValue("content", "Welcome to the LocalGov StarterKit for umbraco, have a look around and see how you could use umbraco to build your own site.");
                home.SetValue("siteName", "LocalGov Starter kit");
